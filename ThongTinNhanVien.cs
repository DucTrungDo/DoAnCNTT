...
namespace QLNhanSuDVSX
{
    public partial class ThongTinNhanVien : Form
    {
        ...
        void LoadData()
        {
            ...
            using (var QLNS = new QLNhanSuDVSXs())
            {
                using (var Transaction = QLNS.Database.BeginTransaction())
                {
                    try
                    {
                        var listnv = QLNS.NhanViens.Select(x => new { x.MaNS, x.HoTen, x.GioiTinh, x.QueQuan, x.NgSinh, x.TrinhĐoHV, x.SĐT, x.DiaChi, x.CongViecDamNhiem, x.ChucVuNV, x.PhongBan, x.Luong, x.SoLanThuong }).ToList();
        ...
        private void btnLuu_Click(object sender, EventArgs e)
        {
            ...
            string cVDamNhiem = txtCVDamNhiem.Text.ToString();
            string chucVu = txtChucVu.Text.ToString();
            string phongBan = txtPhongBan.Text.ToString();
            ...
            using (var QLNS = new QLNhanSuDVSXs())
            {
                using (var Transaction = QLNS.Database.BeginTransaction())
                {
                    ...
                    if (check == 1)
                    {
                        try
                        {
                            NhanVien.InsertNewRowsNhanVien(maNS, hoTen, gioiTinh, queQuan, ngSinh, trinhDoHV, sdt, diaChi, cVDamNhiem, chucVu, phongBan, null, soLanThuong);
                    ...
                    if (check == 2)
                    {
                        try
                        {
                            ...
                            var result = QLNS.NhanViens.Where(cn => cn.MaNS.Equals(strMaNS)).Single();
                            // Cập nhật
                            result.MaNS = maNS;
                            result.HoTen = hoTen;
                            ...
                            result.CongViecDamNhiem = cVDamNhiem;
                            result.ChucVuNV = chucVu;
                            result.PhongBan = phongBan;
                    ...
                    if (check == 3)
                    {
                        ...
                        var result2 = QLNS.DangKy_TeamBuildings.Where(nv1 => nv1.MaNS.Equals(strMaNV)).SingleOrDefault();
                        if (result1 != null)
                        {
                        ...
                        if (result2 != null)
                        {
                            QLNS.DangKy_TeamBuildings.Remove(result2);
                        }
                        // Xóa bản ghi NhanVien
                        var result3 = QLNS.NhanViens.Where(nv => nv.MaNS.Equals(strMaNV)).Single();
                        QLNS.NhanViens.Remove(result3);
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            this.txtCVDamNhiem.Text = dgvNhanSu.Rows[r].Cells[8].Value.ToString();
            this.txtChucVu.Text = dgvNhanSu.Rows[r].Cells[9].Value.ToString();
            this.txtPhongBan.Text = dgvNhanSu.Rows[r].Cells[10].Value.ToString();
            this.txtSoLanThuong.Text = dgvNhanSu.Rows[r].Cells[12].Value.ToString();
            // Cho thao tác trên các nút Lưu / Hủy / Panel
            this.btnLuu.Enabled = true;
            this.btnHuy.Enabled = true;
            this.panel1.Enabled = true;
